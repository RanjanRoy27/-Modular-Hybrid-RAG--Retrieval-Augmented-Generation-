...
def run_rag():
    ...
    while True:
        ...
        try:
            ...
            if chat_history:
                rephrase_res = rephrase_chain.invoke({"chat_history": chat_history, "question": user_input})
                search_query = core.clean_ai_content(rephrase_res.content)
                print(f"  [Debug] Search Query: {search_query}")

            docs = retriever.invoke(search_query)
            context = "\n\n".join([doc.page_content for doc in docs])
            response = qa_chain.invoke({"context": context, "chat_history": chat_history, "question": user_input})
