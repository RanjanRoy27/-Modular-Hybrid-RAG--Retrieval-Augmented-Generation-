...
def build_agent(domain=None):
    llm = core.get_llm()
    config = domain.get_config() if domain else {}
    top_k = int(config.get("top_k", 5))
    system_prompt = (
        domain.get_system_prompt()
        if domain
        else "You are a Smart Assistant with deep knowledge of the company's files."
    )
    ...
    def semantic_search(query: str) -> str:
        ...
        docs = reranker.rerank(query, initial_docs, top_k=top_k)
